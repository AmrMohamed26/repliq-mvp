 ...
 * Uses full-height negative margin overlay (Gmail ignores small offsets).
 */

const PLAY_SIZE_PX = 48;

/** White circle + border triangle — matches results UI, scaled for email. */
function emailPlayButtonDiv(): string {
  const line = PLAY_SIZE_PX;
  return `<div style="width:${line}px;height:${line}px;border-radius:50%;background-color:#ffffff;margin:0 auto;box-shadow:0 4px 20px rgba(0,0,0,0.35);text-align:center;line-height:${line}px;font-size:0;mso-line-height-rule:exactly;"><span style="display:inline-block;width:0;height:0;border-style:solid;border-width:8px 0 8px 14px;border-color:transparent transparent transparent #0a0a0a;margin-left:5px;vertical-align:middle;line-height:0;font-size:0;"></span></div>`;
}
  ...
  fetch("http://127.0.0.1:7489/ingest/874f54e3-af15-42bb-a33a-e094f9419f9f", {
    ...
    body: JSON.stringify({
      ...
      data: {
        ...
        overlayMarginTop,
        thumbHeight: height,
        layout: "single-td-full-height-overlay",
        hasBorderTriangle: playDiv.includes("border-width:8px"),
      },
      timestamp: Date.now(),
    }),
  }).catch(() => {});
