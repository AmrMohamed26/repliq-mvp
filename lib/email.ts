...
import { emailAvatarProxyUrl, watchPageUrl } from "@/lib/app-url";
import {
...
export interface EmailBodyParams {
  ...
  watchUrl: string;
  thumbnailUrl?: string;
  /** Talking-head frame for bottom-left overlay. */
  avatarUrl?: string;
}
...
export function getEmailHtmlForResult(
  ...
  const avatarUrl = emailAvatarProxyUrl(result.id, requestOrigin);
...
export function buildEmailBody({
  ...
  const mediaSection = thumbnailUrl
    ? emailThumbnailWithPlayOverlay(watchUrl, thumbnailUrl, w, h, avatarUrl)
